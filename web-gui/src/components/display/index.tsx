...
const Display: FC<props> = ({ }) => {
    let enable: boolean = useSelector((state: any) => state.settings.display.enable, shallowEqual);
    let transport: string = "test"; //= useSelector((state: any) => state.settings.connection.transport, shallowEqual);
    ...
    return (<div>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "flex-start", width: "200px", background: "linear-gradient(130deg,#05101efe, #061834f0)", borderRadius: "10px", padding: "10px", margin: "5px", boxShadow: '8px 8px 3px -3px rgba(0,0,0,.09)' }}>
            ...
            <FormControlLabel control={<Android12Switch
                checked={enable}
                onChange={(e) => dispatch(actions.displatyEnable())}
